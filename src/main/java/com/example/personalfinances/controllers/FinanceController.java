...
public class FinanceController {
    ...
    public ResponseEntity<BalanceResponse> updateBalance(@Valid @RequestBody BalanceUpdateRequest request) {
        BalanceResponse response = userFinanceService.updateBalance(
                request.getUserName(),
                request.getBalance()
        );
        return ResponseEntity.ok(response);
